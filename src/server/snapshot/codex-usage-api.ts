...
export async function fetchCodexUsageFromApi(
  ...
  if (!auth) {
    return { ok: false, code: 'AUTH_REQUIRED', message: 'No codex OAuth credentials found (run codex login)' };
  }
  ...
  try {
    ...
    return {
      ...
      message: aborted ? 'Codex usage API timed out' : 'Codex usage API request failed',
    };
  }

  // Status codes only in error messages — response bodies never reach snapshot state.
  if (response.status === 401 || response.status === 403) {
    return { ok: false, code: 'AUTH_REQUIRED', message: 'Codex usage API rejected the OAuth token' };
  }
  if (!response.ok) {
    return { ok: false, code: 'HTTP_ERROR', message: `Codex usage API returned HTTP ${response.status}` };
  }
  ...
  try {
    ...
    return { ok: false, code: 'MALFORMED_OUTPUT', message: 'Codex usage API returned non-JSON output' };
  }

  const snapshot = parseCodexUsageResponse(body);
  if (!snapshot) {
    return { ok: false, code: 'MALFORMED_OUTPUT', message: 'Codex usage API response missing rate_limit' };
  }
